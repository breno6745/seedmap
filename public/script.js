...
const STRUCTURE_META = {
  village: { name: "Vila", icon: "icons/village.png" },
  desert: { name: "Templo do deserto", icon: "icons/desert-temple.png" },
  jungle: { name: "Templo da selva", icon: "icons/jungle-temple.png" },
  witch: { name: "Cabana de bruxa", icon: "icons/witch-hut.png" },
  monument: { name: "Monumento oceânico", icon: "icons/monument.png" },
  stronghold: { name: "Stronghold", icon: "icons/stronghold.png" },
  mineshaft: { name: "Mina abandonada", icon: "icons/mineshaft.png" },
  spawn: { name: "Spawn do mundo", icon: "icons/spawn.png" }
};
...
function drawWorldSpawn(ratio) {
  ...
  const image = loadIcon("icons/spawn.png");

  if (!image.complete) return;

  const size = Math.max(25, Math.min(38, 28 * ratio));
  ...
  context.drawImage(
    image,
    point.x * ratio - size / 2,
    point.y * ratio - size / 2,
    size,
    size
  );
  context.restore();
...
function findMineshafts(bounds) {
  ...
  /*
   * Minas são numerosas. Para não travar o navegador, só mostramos
   * quando a área visível não passa de 65.536 chunks.
   */
  if (chunkWidth * chunkHeight > 65536) return [];

  const maximum = Math.min(4096, chunkWidth * chunkHeight);
