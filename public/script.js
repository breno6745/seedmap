...
const STRUCTURE_META = {
  village: { name: "Vila", icon: "markers/mc-village-v3.png" },
  desert: { name: "Templo do deserto", icon: "markers/mc-desert-temple-v3.png" },
  jungle: { name: "Templo da selva", icon: "markers/mc-jungle-temple-v3.png" },
  witch: { name: "Cabana de bruxa", icon: "markers/mc-witch-hut-v3.png" },
  monument: { name: "Monumento oceânico", icon: "markers/mc-monument-v3.png" },
  mineshaft: { name: "Mina abandonada", icon: "markers/mc-mineshaft-v3.png" },
  spawn: { name: "Spawn do mundo", icon: "markers/mc-spawn-v3.png" }
};
...
const mineshaftTileCache = new Map();
...
function drawMap() {
  if (!moduleInstance) {
    ...
    loadingElement.textContent = "Carregando Cubiomes…";
    return;
  }
...
function drawWorldSpawn(ratio) {
  ...
  const image = loadIcon("markers/mc-spawn-v3.png");

  drawMarkerBadge(image, point.x, point.y, ratio, false);
}
...
function loadIcon(path) {
  ...
  image.onerror = () => {
    console.error(`Não foi possível carregar o marcador: ${path}`);
  };
...
function mineshaftTileKey(tileX, tileZ) {
  return `${worldSeed}:${tileX}:${tileZ}`;
}

function getMineshaftTile(tileX, tileZ) {
  const key = mineshaftTileKey(tileX, tileZ);

  if (mineshaftTileCache.has(key)) {
    return mineshaftTileCache.get(key);
  }

  const chunkX = tileX * MINE_TILE_CHUNKS;
  const chunkZ = tileZ * MINE_TILE_CHUNKS;
  const maximum = 2048;
  const pointer = moduleInstance._malloc(maximum * 2 * 4);

  try {
    const count = moduleInstance._fill_mineshafts(
      chunkX,
      chunkZ,
      MINE_TILE_CHUNKS,
      MINE_TILE_CHUNKS,
    ...
    for (let index = 0; index < count; index++) {
      result.push({
        x: moduleInstance.HEAP32[start + index * 2],
        z: moduleInstance.HEAP32[start + index * 2 + 1],
        type: "mineshaft"
      });
    }

    mineshaftTileCache.set(key, result);
    return result;
  } finally {
    moduleInstance._free(pointer);
  }
...
function findMineshafts(bounds) {
  const minChunkX = floorDiv(bounds.left, 16) - 1;
  const maxChunkX = floorDiv(bounds.right, 16) + 1;
  const minChunkZ = floorDiv(bounds.top, 16) - 1;
  const maxChunkZ = floorDiv(bounds.bottom, 16) + 1;

  const minTileX = floorDiv(minChunkX, MINE_TILE_CHUNKS);
  const maxTileX = floorDiv(maxChunkX, MINE_TILE_CHUNKS);
  const minTileZ = floorDiv(minChunkZ, MINE_TILE_CHUNKS);
  const maxTileZ = floorDiv(maxChunkZ, MINE_TILE_CHUNKS);

  const result = [];

  for (let tileZ = minTileZ; tileZ <= maxTileZ; tileZ++) {
    for (let tileX = minTileX; tileX <= maxTileX; tileX++) {
      for (const marker of getMineshaftTile(tileX, tileZ)) {
        if (
          marker.x >= bounds.left &&
          marker.x <= bounds.right &&
          marker.z >= bounds.top &&
          marker.z <= bounds.bottom
        ) {
          result.push(marker);
        }
      }
...
function findStructures(bounds) {
  ...
  try {
    for (const typeName of activeStructures()) {
      if (typeName === "mineshaft") {
        markers.push(...findMineshafts(bounds));
        continue;
      }
...
function drawMarkerBadge(image, screenX, screenY, ratio, isMine = false) {
  if (!image.complete || image.naturalWidth === 0) return;

  const badgeCssSize = isMine ? 30 : 34;
  const badgeSize = badgeCssSize * ratio;
  const iconSize = (badgeCssSize - 8) * ratio;
  const x = screenX * ratio;
  const y = screenY * ratio;

  context.save();

  context.fillStyle = "rgba(19, 23, 28, 0.90)";
  context.strokeStyle = isMine ? "#d6a857" : "#e9eef5";
  context.lineWidth = 2 * ratio;

  context.beginPath();
  context.arc(x, y, badgeSize / 2, 0, Math.PI * 2);
  context.fill();
  context.stroke();
...
canvas.addEventListener("pointermove", event => {
  ...
  if (marker) {
    const name = STRUCTURE_META[marker.type]?.name || marker.type;
...
function preloadMarkerIcons() {
  const paths = new Set([
    ...Object.values(STRUCTURE_META).map(meta => meta.icon),
    "markers/mc-spawn-v3.png"
  ]);
