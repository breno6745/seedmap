                    ...
                    int maximum, int *output)
{
    ...
    /*
     * A implementação do Cubiomes percorre coordenadas inclusivas:
     * chunk inicial X/Z até chunk final X/Z.
     */
    int chunk_x_end = chunk_x + chunk_w - 1;
    int chunk_z_end = chunk_z + chunk_h - 1;
    ...
    {
        /*
         * getMineshafts devolve a origem do chunk em blocos.
         * +8 marca aproximadamente o centro do chunk.
         */
        output[i * 2] = positions[i].x + 8;
        output[i * 2 + 1] = positions[i].z + 8;
    }
